...
describe('InterchainProposalExecutor', function () {
  ...
  describe('withdraw', () => {
    it("should revert when the caller isn't the owner", async () => {
      ...
      await expect(
        executor
          .connect(signer2)
          .withdraw(signer2.address, ethers.utils.parseEther('1')),
      ).to.be.reverted;
    });

    it("should revert when the executor doesn't have any native tokens", async function () {
      await expect(
        executor.withdraw(signerAddress, ethers.utils.parseEther('1')),
    ...
    it('should be able to withdraw native tokens', async function () {
      ...
      const withdrawAmount = sentAmount.div(2);

      await expect(executor.withdraw(recipient.address, withdrawAmount))
        .to.emit(executor, 'Withdrawn')
        .withArgs(recipient.address, withdrawAmount);

      expect(await ethers.provider.getBalance(executor.address)).to.eq(withdrawAmount);
  ...
  describe('withdrawAll', () => {
    it("should revert when the caller isn't the owner", async () => {
      const [, signer2] = await ethers.getSigners();
      await expect(executor.connect(signer2).withdrawAll(signerAddress)).to.be
        .reverted;
    });

    it('should be able to withdraw all native tokens', async function () {
      const sentAmount = ethers.utils.parseEther('1');
      const recipient = ethers.Wallet.createRandom();
      ...
      await expect(executor.withdrawAll(recipient.address))
        .to.emit(executor, 'Withdrawn')
        .withArgs(recipient.address, sentAmount);
